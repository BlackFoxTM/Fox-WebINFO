...
def checker(url):
    ...
    return ("[-] Web Server : %s\n[-] Font Script : %s\n[-] ecommerce : %s\n[-] CMS : %s\n[-] Programmed Lang With : %s\n[-] Blogs : %s" % (operation.get("web-servers")[0] , operation.get("font-scripts")[0] , operation.get("ecommerce")[0] , operation.get("cms")[0] , operation.get("programming-languages")[0] , operation.get("blogs")[0]))
